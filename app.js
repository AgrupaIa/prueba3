...
document.getElementById('clusterBtn').addEventListener('click', async function() {
    ...
    try {
        // [Mantén todo el código anterior hasta donde se obtienen los grupos...]
        
        // Después de obtener los grupos, muestra una tabla detallada
        const analysisDiv = document.getElementById('analysisContent');
        analysisDiv.innerHTML = `
            <h3>Clasificación de Clientes</h3>
            <p><strong>Columna analizada:</strong> ${columnToAnalyze}</p>
                    ...
                    <tbody>
                        ${processedData.slice(0, 50).map(row => `
                            <tr>
                                ...
                                <td>${row[columnToAnalyze]}</td>
                                <td data-group="${row.Grupo}">${row.Grupo}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
            ...
            <div class="groups-summary">
                ${Object.entries(result).map(([group, count]) => `
                    <div class="group-summary" data-group="${group}">
                        <h5>${group}</h5>
                        <p>${count} clientes (${((count / rawData.length) * 100).toFixed(1)}%)</p>
                        <p>Características: ${getGroupDescription(group, columnToAnalyze)}</p>
                    </div>
                `).join('')}
            </div>
            
            <button onclick="renderTableData()" class="btn">Ver en Tabla Principal</button>
        `;
